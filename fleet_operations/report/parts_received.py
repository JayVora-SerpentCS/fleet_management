...
class StockPickingReport(models.TransientModel):
    ...
    name = fields.Char(default="Genric Report.xls")
    ...
    def print_received_part_xlsx_report(self):
        ...
        for part in self:
            part.write({"name": False, "file": False})
            ...
            part.write({"name": "Received Parts.xls", "file": res})
            return {
                "view_type": "form",
                "view_mode": "form",
        ...
        for picking in stock_picking_ids:
            ...
            for line in picking.move_lines:
                if picking.state == "done" and purchase_id:
                    ...
                    worksheet.write(row, 1, purchase_id and purchase_id.name or "")
                    worksheet.write(
                        row, 2, picking.product_id and picking.product_id.default_code
                    )
                    worksheet.write(
                        row, 3, picking.product_id and picking.product_id.name or ""
                    )
                    worksheet.write(
                        row,
                        4,
                        picking.product_id
                        and picking.product_id.vehicle_make_id
                        and picking.product_id.vehicle_make_id.name
                        or "",
                    )
                    worksheet.write(
                        row, 5, picking.partner_id and picking.partner_id.name or ""
                    )
                    worksheet.write(row, 6, line.product_qty or 0.0)
                    worksheet.write(row, 7, line.price_unit or 0.0)
                    worksheet.write(row, 8, (line.price_unit * line.product_qty))
                    worksheet.write(row, 9, picking.user_id and picking.user.name or "")
                    worksheet.write(row, 10, picking and picking.partner_id.name or "")
                    if picking and picking.location_dest_id:
                        worksheet.write(row, 11, picking.location_dest_id.name)
                    else:
