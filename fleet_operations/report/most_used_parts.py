...
class MostUsedPartsXlsx(models.AbstractModel):
    ...
    def get_most_used_parts(self, date_range, top_no):
        ...
        if work_order_ids:
            for p_line in work_order_ids:
                if p_line.fleet_service_id and p_line.fleet_service_id.state == "done":
                    if used_dict.get(p_line.product_id.id, False):
                        used_dict[p_line.product_id.id]["qty"] += p_line.qty or 0.0
                        ...
                        used_dict[p_line.product_id.id] = {
                            "part_no": p_line.product_id
                            and p_line.product_id.default_code
                            or "",
                            "part_name": p_line.product_id
                            and p_line.product_id.name
                            or "",
                            "vehicle_make": p_line.product_id
                            and p_line.product_id.vehicle_make_id
                            and p_line.product_id.vehicle_make_id.name
                            or "",
                            "qty": p_line.qty or 0.0,
                            "qty_available": p_line.product_id
                            and p_line.product_id.qty_available,
                            "uom": p_line.product_id
                            and p_line.product_id.uom_id
                            and p_line.product_id.uom_id.name
                            or "",
                            "unit_cost": p_line.product_id
                            and p_line.product_id.standard_price
                            or 0.0,
                            "total_cost": p_line.qty * p_line.product_id.standard_price,
                        }

        for part_data in used_dict.itervalues():
            part_list_data.append(part_data)
        if part_list_data:
            part_list_data = sorted(
                part_list_data, key=lambda k: k["qty"], reverse=True
            )
        else:
            msg = _(
                "Warning! \n\
    ...
    def generate_xlsx_report(self, workbook, data, parts):
        ...
        line_col = 0
        counter = 1
        for parts_value in self.get_most_used_parts(
            data["form"], data["form"]["top_no"]
        ):
            worksheet.write(line_row, line_col, counter, border)
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("part_no"), border)
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("part_name"), border)
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("vehicle_make"), border)
            line_col += 1
            worksheet.write(
                line_row, line_col, parts_value.get("qty_available"), border
            )
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("qty"), border)
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("uom"), border)
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("unit_cost"), border)
            line_col += 1
            worksheet.write(line_row, line_col, parts_value.get("total_cost"), border)
            line_row += 1
            line_col = 0
            counter += 1
        row = line_row + 1
