...
class FleetHistorySummary(models.TransientModel):
    ...
    name = fields.Char(default="generic summary.xls")
...
class PrintFleetHistory(models.TransientModel):
    ...
    def print_xlsx_report(self):
        for rep in self:
            ...
            if rep.sel_report == "history":
                ...
                ret_dict.update(
                    {"name": _("Fleet History Report"), "res_id": module_rec.id}
                )
                ...
                module_rec = self.env["excel.fleet.report"].create(vals)
                ret_dict.update(
                    {"name": _("Fleet Listing Report"), "res_id": module_rec.id}
                )
            elif rep.sel_report == "pending_repairs":
                pending_repair_obj = self.env[
                ...
                module_rec = self.env["excel.fleet.report"].create(vals)
                ret_dict.update(
                    {"name": _("Fleet Pending Repairs Report"), "res_id": module_rec.id}
                )
            elif rep.sel_report == "pending_repair_summary":
                pending_repair_summary_obj = self.env[
                    "report.fleet_operations.fleet.pending.xls"
                ]
                res = pending_repair_summary_obj.generate_pending_summary_xlsx_report(
                    res, obj
                )
                ...
                ret_dict.update(
                    {
                        "name": _("Fleet Pending Repair Summary Report"),
                        "res_id": module_rec.id,
                    }
                )
            elif rep.sel_report == "complete_stage":
                complete_stage_obj = self.env[
                ...
                module_rec = self.env["excel.fleet.report"].create(vals)
                ret_dict.update(
                    {"name": _("Fleet Complete Stage Report"), "res_id": module_rec.id}
                )
            elif rep.sel_report == "nex_ser_odometer":
                ser_odometer_obj = self.env[
                ...
                ret_dict.update(
                    {
                        "name": _("Next Service Odometer Repairs Report"),
                        "res_id": module_rec.id,
                    }
                )
            elif rep.sel_report == "nex_ser_date":
                ser_date_obj = self.env[
                ...
                module_rec = self.env["excel.fleet.report"].create(vals)
                ret_dict.update(
                    {"name": _("Next Service Date Report"), "res_id": module_rec.id}
                )
            return ret_dict
    ...
    def print_fleet_history_xlsx_report(self, res=False, fleet_history=False):
        ...
        for obj in fleet_history:
            ...
            worksheet.write(row, 3, obj.driver_id and obj.driver_id.name or "", border)
            row += 1
            worksheet.write(row, 0, "Vehicle Type :", format1)
            worksheet.write(
                row, 1, obj.vechical_type_id and obj.vechical_type_id.name or "", border
            )
            ...
            worksheet.write(
                row, 1, obj.vehical_color_id and obj.vehical_color_id.name or "", border
            )
                ...
                for repair_line in order.repair_line_ids:
                    if repair_line.complete is True:
                        ...
                        worksheet.write(
                            line_row,
                            line_col,
                            repair_line.repair_type_id
                            and repair_line.repair_type_id.name
                            or "",
                            border,
                        )
                        line_col += 1
                        worksheet.write(
                            line_row,
                            line_col,
                            repair_line.categ_id and repair_line.categ_id.name or "",
                            border,
                        )
                        line_col = 0
                        line_row += 1
