...
class NextServiceDate(models.AbstractModel):
    ...
    def generate_service_date_xlsx_report(self, res, next_service):
        ...
        for obj in next_service:
            ...
            worksheet.write(line_row, line_col, ser_date or "", style1)
            line_col += 1
            next_date = ""
            if obj.next_service_date:
                next_date = format_date(
                    self.env,
                    obj.next_service_date,
                    self._context.get("lang"),
                    date_format=False,
                )
            worksheet.write(line_row, line_col, next_date or "", style1)
            line_col += 1
            worksheet.write(
                line_row,
                line_col,
                obj.vehicle_location_id and obj.vehicle_location_id.name or "",
                border,
            )
            line_col = 0
            line_row += 1
