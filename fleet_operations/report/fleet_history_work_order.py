...
class FleetHistoryWorkOrder(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workorder):
        ...
        for order in workorder:
            ...
            row += 1
            worksheet.write(row, 1, order.team_id and order.team_id.name or "")
            ...
            worksheet.write(
                ...
                7,
                order.vehicle_id
                and order.vehicle_id.driver_id
                and order.vehicle_id.driver_id.name
                or "",
            )

            row += 1
            worksheet.write(row, 0, "Vehicle ID :", tot)
            worksheet.write(row, 1, order.vehicle_id and order.vehicle_id.name or "")
            worksheet.write(row, 2, "Kilometer :", tot)
            worksheet.write(row, 3, order.odometer or "")
            worksheet.write(row, 6, "Driver Contact No :", tot)
            worksheet.write(
                row, 7, order.vehicle_id and order.vehicle_id.driver_contact_no or ""
            )
            row += 1
            worksheet.write(row, 0, "VIN No :", tot)
            worksheet.write(row, 1, order.vehicle_id and order.vehicle_id.vin_sn or "")
            worksheet.write(row, 2, "Engine No :", tot)
            worksheet.write(
                row, 3, order.vehicle_id and order.vehicle_id.engine_no or ""
            )
            worksheet.write(row, 6, "Registration State :", tot)
            worksheet.write(
                row,
                7,
                order.vechical_location_id and order.vechical_location_id.name or "",
            )
            row += 1
            ...
            worksheet.write(
                ...
                1,
                order.vehicle_id
                and order.vehicle_id.vechical_type_id
                and order.vehicle_id.vechical_type_id.name
                or "",
            )
            worksheet.write(row, 2, "Plate No :", tot)
            worksheet.write(
                row, 3, order.vehicle_id and order.vehicle_id.license_plate or ""
            )
            row += 1
            ...
            worksheet.write(
                ...
                1,
                order.vehicle_id
                and order.vehicle_id.vehical_color_id
                and order.vehicle_id.vehical_color_id.name
                or "",
            )
            row += 3
            ...
            for repair_line in order.repair_line_ids:
                if repair_line.complete is True:
                    ...
                    line_col += 1
                    worksheet.write(line_row, line_col, repair_line.target_date)
                    line_col += 1
                    worksheet.write(
                        line_row,
                        line_col,
                ...
                if repair_line.complete is False:
                    ...
                    line_col += 1
                    worksheet.write(
                        row, 2, repair_line.categ_id and repair_line.categ_id.name or ""
                    )
                    line_col += 1
