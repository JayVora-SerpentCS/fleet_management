...
class WoOver10DaysXlsx(models.AbstractModel):
    ...
    def get_wo_over_10days(self, work_orders):
        ...
        for wk_order in work_orders:
            if wk_order.state == "done":
                diff = (
                    datetime.strptime(
                        str(wk_order.date_close), DEFAULT_SERVER_DATE_FORMAT
                    )
                    - datetime.strptime(
                        str(wk_order.date_open), DEFAULT_SERVER_DATE_FORMAT
                    )
                ).days
                if diff > 10:
    ...
    def generate_xlsx_report(self, services):
        ...
        for line in res1:
            ...
            worksheet.write(row, 5, self.get_over_wo_repair_perform(line, line.state))
            worksheet.write(row, 6, line.etic and line.date_complete or "")
            row += 1
            counter += 1
        row += 5
        fp = io.BytesIO()
