...
class RepairLineSummary(models.AbstractModel):
    ...
    def get_repair_line_detail(self, date_range):
        ...
        work_order_ids = work_order_obj.search(
            [("date", ">=", start), ("date", "<=", end), ("state", "=", "done")]
        )
        repair_line_data = []
        repair_l_dic = {}
        if not work_order_ids:
            msg = _("Warning! No Work order found between the selected date !!")
            raise UserError(msg)
        if work_order_ids:
        ...
        if repair_line_data:
            repair_line_data = sorted(repair_line_data, key=lambda k: k["repair_type"])
