...
class FleetOutstandingWO(models.AbstractModel):
    ...
    def generate_xlsx_report(self, product):
        ...
        for obj in product:
            ...
            line_col += 1
            worksheet.write(line_row, line_col, obj.name or "")
            line_col += 1
            worksheet.write(
                line_row, line_col, obj.vehicle_id and obj.vehicle_id.name or ""
            )
            line_col += 1
            worksheet.write(
                line_row, line_col, obj.vehicle_id and obj.vehicle_id.vin_sn or ""
            )
            line_col += 1
            worksheet.write(
                line_row,
                line_col,
                obj.vehicle_id
                and obj.vehicle_id.f_brand_id
                and obj.vehicle_id.f_brand_id.name
                or "",
            )
            line_col += 1
            worksheet.write(
                ...
                line_col,
                obj.vehicle_id
                and obj.vehicle_id.model_id
                and obj.vehicle_id.model_id.name
                or "",
            )
            line_col += 1
            worksheet.write(line_row, line_col, self.get_wo_status(obj.state) or "")
            line_col += 1
            worksheet.write(line_row, line_col, obj.etic and obj.date_complete or "")
            line_col += 1
            worksheet.write(line_row, line_col, self.get_work_incomplete(obj) or "")
            line_col = 0
            line_row += 1
            counter += 1
