...
class TestRegistration(TransactionCase):
    ...
    def test(self):
        ...
        self.vehicle_obj.get_driver_id_no()

        vehicle_registration_state = registration_state_obj.search(
            [("name", "=", "Gujarat")]
        )

        vehicle_color = vehicle_color_obj.search([("name", "=", "BLACK")])
        vehicle_type_obj.search([("name", "=", "Car")])

        self.driver = driver_obj.create({"name": "Shivam", "email": "shivam@gmail.com"})
