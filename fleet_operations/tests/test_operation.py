...
class TestOperation(TransactionCase):
    def setUp(self):
        ...
        self.create_res_user = self.env["res.users"].sudo()
        ...
        self.vehicle_color = self.vehicle_color_obj.search([("name", "=", "BLACK")])

    def test_vehicle_operation(self):

        # create car maker
        self.create_make_1 = self.create_make.create({"name": "RJ Makers"})
        ...
        self.assertTrue(self.create_res_user_owner_1, "Vehicle owner not create")
        # create vehicle
        ...
        # create repair type
        self.repair_type = self.repair_type_obj.create({"name": "Test Engine Repair"})
        ...
        # create color
        self.color = self.vehicle_color_obj.create({"name": self.vehicle_color.id})
        self.assertTrue(self.color, "Vehicle Color not create")
        # create vehicle service
        self.vehicle_service = self.service_type_obj.create({"name": "Washing"})
        self.assertTrue(self.vehicle_service, "Vehicle Service not create")
        # create Driver
        self.driver = self.driver_obj.create(
            {"name": "jethalal", "email": "jethalal@gmail.com", "phone": 9898981213}
        )
