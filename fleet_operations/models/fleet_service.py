...
class service_category(models.Model):
    ...
    def copy(self, default=None):
        if not default:
            default = {}
        raise Warning('User Error!', 'You can\'t duplicate record!')
        return super(service_category, self).copy(default=default)

    @api.multi
    def unlink(self):
        raise Warning('User Error!', 'You can\'t delete record !')
...
class fleet_vehicle_log_services(models.Model):
    ...
    def copy(self, default=None):
        if not default:
            default = {}
        raise Warning(_('User Error!'), _('You can\'t duplicate record!'))
        ...
        for rec in self:
            if rec.state != 'draft':
                raise Warning(_('User Error!'),
                              _('You can\'t delete Work Order which \
    ...
    def get_vehicle_info(self):
        if self.vehicle_id:
            ...
            self.main_type = vehicle.main_type,
        ...
        for work_order in self:
            if work_order.vehicle_id:
                if work_order.vehicle_id.state == 'write-off':
                    raise Warning("Warning!", _("You can\'t confirm this \
                            work order which vehicle is in write-off state!"))
                elif work_order.vehicle_id.state == 'in_progress':
                    raise Warning("Warning!", _("Previous work order is not \
                    ...
                    raise Warning("Warning!", _("Confirm work order can only \
    ...
    def action_done(self):
        ...
        if not increment_ids:
            raise Warning(_('Configuration Error!'), _("Next Increment \
        ...
        if not next_service_day_ids:
            raise Warning(_('Configuration Error!'), _("Next service days is \
        ...
        for work_order in self:
            ...
            if work_order.odometer == 0:
                raise Warning(_('User Error!'), _("Please set the current \
            ...
            for product in work_order.parts_ids:
                ...
                if product.old_part_return:
                    ...
                    if not scrap_dest_loc_ids:
                        raise Warning(_('User Error!'), _("There is no \
                                locatin defined for scrapped parts please \
                                define location with Scrapped!"))
                    if work_order.team_id:
                        scrap_source_loc_id = work_order.team_id and \
                                        work_order.team_id.id or False
                    else:
                        raise Warning(_('User Error!'), _("There is no team \
...
#                if not reopen_ids:
#                    raise Warning(_('Configuration Error!'), _('Work Order \
#                    Re-open Days is not configured for %s please configure it\
#                    from configuration menu.') % (order.vehicle_id.name))
#                open_days = reopen_ids[0]
                if order.vehicle_id.state == 'write-off':
                    raise Warning("Warning!", _("You can\'t Re-open this \
                            work order which vehicle is in write-off state!"))
                    ...
                    raise Warning("Warning!", _("Re-open work order can \
...
#                self.write({'state': 'confirm'})
#            else:
#                raise Warning(_('User Error!'), _("This Work order is closed \
...
#                        re-open again!" % (open_days.days)))
    ...
    def service_send_mail(self):
        ...
        send_obj = self.env['email.template']
...
#        due_odometer <= odometer OR
        vehicle_ids = [i[0]for i in self._cr.fetchall()]
    ...
    def default_get(self, fields):
        ...
        if self._context.get('active_ids', False):
            for vehicle in vehicle_obj.browse(self._context['active_ids']):
                if vehicle.state == 'write-off':
                    raise Warning("Warning!", _("You can\'t create work order \
                    ...
                    raise Warning("Warning!", _("New work order can only be \
        ...
        if not repair_type_ids:
            raise Warning("Configuration Error!", _("There is no data for \
    ...
    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context=None):
        if not vehicle_id:
            return {}
        vehicle_rec = self.pool.get('fleet.vehicle').browse(
                                        cr, uid, vehicle_id, context=context)
        return {
            'value': {
                'odometer': vehicle_rec.odometer,
                'odometer_unit': vehicle_rec.odometer_unit,
            }
        }
...
class fleet_team(models.Model):
    ...
    @api.multi
    def unlink(self):
        raise Warning(_('User Error!'), _('You can\'t delete record !'))
        ...
        for team in self:
            if team.state == 'close':
                raise Warning("User Error!",
                              _("Loaded parts already Deliver to trip!"))
            if team.state == 'sent':
                raise Warning("User Error!",
                              _("Team is already sent!"))
            if team.state != 'open':
                raise Warning("User Error!",
                              _("Please Open your trip before deliver it!"))
            if not team.allocate_part_ids:
                raise Warning("User Error!",
                              _("Please load the parts before deliver it!"))
    ...
    def get_return_from_trip(self):
        ...
        for team in self:
            if team.state != 'sent':
                raise Warning("User Error!", _("Please send your trip \
                                             before getting return it!"))
            if not team.allocate_part_ids:
                raise Warning("User Error!", _("parts are not loaded!"))
            if not team.outgoing_ship_ids:
                raise Warning("User Error!", _("parts are not Delivered \
    ...
    def close_trip(self):
        ...
        for team in self:
            if not team.is_return:
                raise Warning("User Error!", _("Please Fill the Used Part \
...
#            if not reopen_ids:
#                raise Warning(_('Configuration Error!'), _('Work Order Re-open\
...
class trip_parts_history_details(models.Model):
    ...
    def _get_available_qty(self):
        for rec in self:
            ...
            if available_qty < 0:
                raise Warning(_('User Error!'), _('Quantity Available \
        ...
        for wizard_line in partial.move_lines:
            ...
            if line_uom.factor and line_uom.factor != 0:
                if float_compare(qty_in_line_uom, wizard_line.product_qty,
                    ...
                    raise Warning(_('Warning!'), _('The unit of measure \
            ...
            if move_id:
                ...
                if float_compare(qty_in_initial_uom, without_rounding_qty,
                    ...
                    raise Warning(_('Warning!'), _('The rounding of the \
...
class stock_transfer_details_items(models.TransientModel):
    ...
    @api.multi
    def do_partial(self):
        stock_picking_in = self.env['stock.picking']
        result = super(stock_transfer_details_items, self).do_partial()
        if not self._context:
            self._context = {}
        if self._context.get('active_model', False) == 'stock.picking' and \
           self._context.get('active_id', False):
            picking_rec = stock_picking_in.browse(self._context['active_id'])
            picking_rec.received_by_id = self._uid
        return result
    ...
    def onchange_product_id(self):
        if self.product_id:
            ...
            if rec.in_active_part:
                ...
                raise Warning(_('User Error!'), _('You can\'t select \
                                        part which is In-Active!'))
            ...
            if rec.qty_available <= 0:
                ...
                self.vehicle_make_id = False
                self.qty = 0.0
                raise Warning(_('User Error!'), _('You can\'t select part \
        ...
        if issue_date_o:
            ...
            if trip_date and return_date:
                if trip_date > issue_date or issue_date > return_date:
                    ...
                    raise Warning(_('Warning!'), _('Please enter \
                ...
                self.issue_date = False
                raise Warning(_('Warning !'), _('Please enter current date \
                                           in issue date!!'))
...
class task_line(models.Model):
    ...
    def _check_used_qty(self):
        for rec in self:
            if rec.qty <= 0:
                raise Warning(_('User Error!'), _('You can\'t \
    ...
    def check_onchange_part_issue_date(self):
        ...
        if 'date_open' in context_keys and self.date_issued:
            ...
            if not self.date_issued >= date_open and \
                ...
                self.date_issued = False
                raise Warning(_('User Error!'), _('You can\t enter \
                ...
                if self._context.get('team_id', False):
                    ...
                    if team_trip_ids:
                        ...
                        if not flag and self.encode_qty < self.qty:
                            self.qty = 0.0
                            raise Warning(_('Warning!'),
                                          _('You can\'t enter used quantity \
...
#        for rec in self:
#            if rec.days <= 0:
#                raise Warning(_('User Error!'), _('Re-Open Days \
#                            Must be Greater than Zero!'))
#        return True
