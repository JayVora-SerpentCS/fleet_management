...
class UpdatePendingRepairHistory(models.Model):
    ...
    vehicle_id = fields.Many2one("fleet.vehicle", "Vehicle ID")
    fmp_id = fields.Char("Vehicle")
    pending_reapir_ids = fields.Many2many(
        "pending.repair.type",
        "update_pending_history_rel",
        "pending_history_id",
        "pending_type_id",
        "Vehicle Pending History",
    )
    user_id = fields.Many2one("res.users", "Applied By", readonly=True)
    state = fields.Selection(
        [("draft", "New"), ("confirm", "Applied")], "Status", default="draft"
    )
    note = fields.Text(translate=True)
    pending_reapir_history_ids = fields.One2many(
                ...
                applied_pending_obj.create(
                    {
                        ...
                        "vehicle_rep_type_id": applied_pending.vehicle_rep_type_id
                        and applied_pending.vehicle_rep_type_id.id
                        or False,
                        "repair_type_id": applied_pending.repair_type_id
                        and applied_pending.repair_type_id.id
                        or False,
                        "name": applied_pending.name or "",
                        "categ_id": applied_pending.categ_id
                        and applied_pending.categ_id.id
                        or False,
                        "issue_date": applied_pending.issue_date,
                        "state": "complete",
                        "user_id": applied_pending.user_id
                        and applied_pending.user_id.id
                        or False,
                    }
                )
...
class PendingAppliedHistory(models.Model):
    ...
    updated_pending_id = fields.Many2one(
        "update.pending.repair.history", "Update Pending"
    )
    ...
    state = fields.Selection(
        [("complete", "Complete"), ("in-complete", "In Complete")], "Status"
    )
