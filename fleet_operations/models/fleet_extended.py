...
class fleet_operations(models.Model):
        ...
        if self:
            if self.state == 'write-off':
                raise Warning(_('User Error!'),
                              _('You can\'t duplicate this record \
    ...
    def set_released_state(self):
        for vehicle in self:
            if vehicle.state == 'complete':
                ...
                raise Warning(_('User Error!'),
                              _('Vehicle status will only set to released \
    ...
    def create(self, vals):
        if not vals.get('model_id', False):
            raise Warning(_('User Error!'),
                          _('Model is not selected for this vehicle!'))
...
class fleet_witten_off(models.Model):
    ...
    def get_usd_currency(self):
        ...
        if not usd_ids:
            raise Warning(_('User Configuration Error!'),
...
class fleet_vehicle_model(models.Model):
    ...
    modelname = fields.Char(string='Model name', size=32, required=True,
                            translate=True)
    brand_id = fields.Many2one('fleet.vehicle.model.brand', string='Make',
                               required=True, help='Brand of the vehicle')

    _sql_constraints = [('model_brand_name_uniq', 'unique(modelname,brand_id)',
...
class fleet_vehicle_advance_search(models.TransientModel):
    ...
    vechical_location_id = fields.Many2one('service.department',
                                           string='Province')
    vechical_department_id = fields.Many2one('stock.location',
                                             string='Department')
    ...
    def get_vehicle_detail_by_advance_search(self):
        ...
        for vehicle in self:
            ...
            if vehicle.vechical_department_id:
                domain += [('last_service_by_id', '=',
                            vehicle.vechical_department_id.id)]
            if vehicle.vin_no:
