        ...
        if self._context.get('work_order_id', False):
            ...
            if not increment_ids:
                raise Warning(_('Configuration Error!'),
                              _("Next Increment Odometer is not set for %s \
          please set it from configuration!") % (work_order.vehicle_id.name))
            ...
            if not next_service_day_ids:
                raise Warning(_('Configuration Error!'), _("Next service \
                            days is not configured for %s please set it \
                            from configuration!") % (wo_obj.vehicle_id.name))
            work_order_vals = {}
            if work_order.odometer == 0:
                raise Warning(_('User Error!'), _("Please set the \
