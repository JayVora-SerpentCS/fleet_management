    ...
    def confirm_wo_forcefully(self):
        ...
        if self._context.get("work_order_id", False):
            ...
            if not increment_ids:
                raise UserError(
                    _(
                        "Next Increment Odometer is not set for %s "
                        "please set it from configuration!"
                    )
            ...
            if work_order.odometer == 0:
                raise UserError(
                    _("Please set the " "current Odometer of vehicle in work order!")
                )
            odometer_increment += work_order.odometer
            next_service_date = work_order.date + timedelta(
                days=next_service_day_ids.days
            )

            if work_order.already_closed:
                for repair_line in work_order.repair_line_ids:
                    if repair_line.complete is False:
                        for repair_line_in_vehicle in work_order.vehicle_id:
                            if (
                                work_order.name == repair_line_in_vehicle.name
                                ...
                                incomplete_rep_id = pending_rep_obj.create(
                                    {
                                        "repair_type_id": repair_line.repair_type_id
                                        and repair_line.repair_type_id.id
                                        or False,
                                        "name": work_order.name or "",
                                        "state": "in-complete",
                                        "user_id": self._uid,
                                        "categ_id": repair_line.categ_id
                                        and repair_line.categ_id.id
                                        or False,
                ...
                for repair_line in work_order.repair_line_ids:
                    if repair_line.complete is False:
                        ...
                        work_order.vehicle_id.write(
                            {
                                "pending_repair_type_ids": [(4, incomplete_rep_id.id)],
                                "state": "complete",
                            }
                        )
