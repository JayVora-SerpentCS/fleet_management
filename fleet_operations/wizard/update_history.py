...
class UpdateEngineInfo(models.TransientModel):
    ...
    previous_engine_no = fields.Char(translate=True)
    new_engine_no = fields.Char(translate=True)
                ...
                engine_history_obj.create(
                    {
                        ...
                        "workorder_id": wiz_data.workorder_id
                        and wiz_data.workorder_id.id
                        or False,
...
class UpdateVinInfo(models.TransientModel):
    ...
    previous_vin_no = fields.Char()
    new_vin_no = fields.Char()
    changed_date = fields.Date(default=fields.Date.today(), string="Change Date")
    note = fields.Text(string="Notes", translate=True)
    temp_bool = fields.Boolean(
        default=True,
        string="Temp Bool for making previous \
    ...
    def default_get(self, fields):
        ...
        if self._context.get("active_id", False):
            vehicle = vehicle_obj.browse(self._context["active_id"])
            res.update(
                {"previous_vin_no": vehicle.vin_sn or "", "vehicle_id": vehicle.id}
            )
        return res
...
class UpdateTireInfo(models.TransientModel):
    ...
    temp_bool = fields.Boolean(
        default=True, string="Temp Bool for making " "previous Tire info readonly"
    )
    workorder_id = fields.Many2one("fleet.vehicle.log.services", string="Work Order")
    vehicle_id = fields.Many2one("fleet.vehicle", string="Vehicle")
    ...
    def check_tire_changed_date(self):
        ...
        for vehicle in self:
            if (
                vehicle.changed_date
                and vehicle.new_tire_issue_date
                and vehicle.changed_date < vehicle.new_tire_issue_date
            ):
                msg = _(
                    "Tire Change Date Should Be " "Greater Than New Tire Issuance Date."
                )
                raise ValidationError(msg)
                ...
                tire_history_obj.create(
                    {
                        ...
                        "previous_tire_issue_date": wiz_data.previous_tire_issue_date
                        or False,
    ...
    def check_new_battery_issue_date(self):
        ...
        for vehicle in self:
            if (
                ...
                and vehicle.new_battery_issue_date
                and vehicle.new_battery_issue_date < vehicle.previous_battery_issue_date
            ):
                msg = _(
                    "New Battery Issuance Date Should Be "
                ...
                battery_history_obj.create(
                    {
                        ...
                        "previous_battery_issue_date": wiz_data.previous_battery_issue_date
                        or False,
                        "new_battery_issue_date": wiz_data.new_battery_issue_date
                        or False,
                        "note": wiz_data.note or "",
                        "changed_date": wiz_data.changed_date,
                        "workorder_id": wiz_data.workorder_id.id or False,
