...
from odoo.tools import format_date, ustr
...
class AccountInvoice(models.Model):
    ...
    is_deposit_inv = fields.Boolean("Is Deposit Invoice")
    is_deposit_return_inv = fields.Boolean("Is Deposit Return Invoice")
...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _create_payments(self):
        ...
        if self._context.get("active_ids", False):
            for invoice in inv_obj.browse(self._context["active_ids"]):
                if invoice.fleet_rent_id:
                    self.write(
                        {
                            "fleet_rent_id": invoice.fleet_rent_id
                            and invoice.fleet_rent_id.id
                            or False
                        }
                    )
        ...
        notes = (
            ...
            + ustr(
                format_date(
                    self.env, fields.Date.today(), self.env.user.lang, date_format=False
                )
            )
        )
        if (
                    ...
                    move.fleet_rent_id.write(
                        {
                            ...
                            "amount_return": move.amount_total - move.amount_residual
                            or 0.0,
                        }
                    )
        return res
