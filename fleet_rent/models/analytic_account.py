...
class AccountAnalyticAccount(models.Model):
    ...
    def _get_deposit(self):
        ...
        for tennancy in self:
            payment_ids = self.env['account.voucher'].search(
...
#         default='kilometers', required=True)
            ...
            if tenancy_rec.acc_pay_dep_rec_id and \
                ...
                acc_pay_form_id = \
                    self.env['ir.model.data'].get_object_reference(
                        'account_voucher',
                        'view_vendor_receipt_dialog_form')[1]
                return {
                    'view_type': 'form',
    ...
    def button_return(self):
        ...
        inv_values = {
            ...
            'account_id': self.tenant_id.property_account_payable.id or False,
    ...
    def cron_property_tenancy(self):
        ...
        template_brw = self.env['email.template'].browse(model_data_id)
        ...
        for tenancy_rec in self:
            for rent_line in tenancy_rec.rent_schedule_ids:
                if rent_line.paid is False and rent_line.move_check is False:
                    raise Warning(
                        _('You can\'t create new rent schedule Please make all related \
                        Rent Schedule entries paid.'))
            rent_obj = self.env['tenancy.rent.schedule']
