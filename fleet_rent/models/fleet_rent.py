...
class PropertyMaintenace(models.Model):
    ...
    def onchanchange_assign(self):
        for data in self:
            data.account_code = data.assign_to.property_account_payable
...
class TenancyRentSchedule(models.Model):
    ...
    def create_invoice(self):
        ...
        inv_values = {
            ...
            self.tenancy_id.tenant_id.property_account_receivable.id or False
        }
        if self.tenancy_id.main_cost:
            inv_values.update({'invoice_line': [(0, 0, inv_line_values),
                                                (0, 0, inv_line_main)]})
        else:
            inv_values.update(
