...
class FleetRentalVehicleHistory(models.TransientModel):
    ...
    def rental_vehicle_history(self):
        ...
        fleet_rental = (
            self.env[self.env.context.get("active_model")].browse(docids) or False
        )
        file = self.generate_xlsx_report(fleet_rental)
        self.write({"name": "Vehicle Rental History.xls", "file": file})
        return {
            "view_type": "form",
            "view_mode": "form",
    ...
    def generate_xlsx_report(self, fleet_rental):
        ...
        for obj in fleet_rental:
            ...
            line_col += 1
            worksheet.write(
                line_row, line_col, obj.vehicle_id and obj.vehicle_id.name or "", border
            )
            line_col += 1
            worksheet.write(line_row, line_col, obj.odometer or "", border)
            line_col += 1
            worksheet.write(
                line_row, line_col, obj.tenant_id and obj.tenant_id.name or "", border
            )
            line_col += 1
            ...
            worksheet.write(
                line_row,
                line_col,
                obj.rent_type_id and obj.rent_type_id.name or "",
                border,
            )
            line_col += 1
            ...
            worksheet.write(
                line_row,
                line_col,
                obj.cancel_by_id and obj.cancel_by_id.name or "",
                border,
            )
            line_col += 1
