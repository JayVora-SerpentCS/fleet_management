            ...
            if rec.rent_type_id and rec.start_date:
                if rec.rent_type_id.renttype == "Months":
                    rec.end_date = rec.start_date + relativedelta(
                        months=int(rec.rent_type_id.duration)
                    )
                if rec.rent_type_id.renttype == "Years":
                    rec.end_date = rec.start_date + relativedelta(
                        years=int(rec.rent_type_id.duration)
                    )
                if rec.rent_type_id.renttype == "Weeks":
                    rec.end_date = rec.start_date + relativedelta(
                        weeks=int(rec.rent_type_id.duration)
                    )
                if rec.rent_type_id.renttype == "Days":
                    rec.end_date = rec.start_date + relativedelta(
                        days=int(rec.rent_type_id.duration)
                    )
                if rec.rent_type_id.renttype == "Hours":
                    rec.end_date = rec.start_date + relativedelta(
                        hours=int(rec.rent_type_id.duration)
                    )
        return True

    start_date = fields.Datetime()
    end_date = fields.Datetime(compute="_compute_create_date", store=True)
    rent_type_id = fields.Many2one("rent.type", string="Rent Type", required=True)
    ...
    def renew_contract(self):
        ...
        if self._context.get("active_id", False):
            for rec in self:
                ...
                for rent in rent_rec:
                    if rent.date_close and rec.start_date:
                        if rec.start_date < rent.date_close:
                            raise UserError(
                                _(
                                    "Start Date should be Greater Than Rent "
                                    "Close Date."
                                )
                            )
                ...
                rent.write(
                    {
                        "rent_type_id": rec.rent_type_id
                        and rec.rent_type_id.id
                        or False,
                        "name": "New",
                    }
                )
